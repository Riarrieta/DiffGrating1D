...
# curves
curve_circle(r;z=0.0) = t -> Point2D(r*cos(t),r*sin(t)+z)
...
function curves_square(N,p;L=2.0,z=0.0)
    # square with corners (L/2,L/2), (-L/2,L/2), (-L/2,-L/2),(L/2,-L/2)
    # shifted by z in the y direction
    @assert iseven(N)
    Δz = Point2D(0.0,z)
    p1 = Point2D(L/2,L/2) + Δz
    p2 = Point2D(-L/2,L/2) + Δz
    p3 = Point2D(-L/2,-L/2) + Δz
    p4 = Point2D(L/2,-L/2) + Δz
